...
def store_Excel(list):
    with open("data/csvdata/微博实时热搜.csv", "a", encoding="utf_8_sig", newline="")as file:
...
def get_cloud():
    str = ""
    with open("data/csvdata/微博实时热搜test.csv", "r", encoding="utf_8_sig", newline="") as file:
...
def analysis_data():
    word_dic = {}
    with open("data/txtdata/user_stop_dict.txt", "r", encoding="utf-8") as file:
        txt = file.read()
        file.close()
    # jieba分词切分数据
